...
export default function PokemonListPage() {
  ...
  // Infinite query for progressive loading with persistence
  // Reset when filter changes
  useEffect(() => {
    // setAllPokemon([]); // Removed as we use derived state
    // setCurrentPage(1); // Removed
    // setIsFullyLoaded(false); // Removed
    // setIsFetchingMore(false); // Removed
  }, [typeFilter, currentLocale]);
  ...
  const {
    ...
    queryKey: ['pokemon-infinite', typeFilter, currentLocale],
    queryFn: async ({ pageParam = 1 }: { pageParam: number }) => {
      console.log('Fetching page:', pageParam, 'Locale:', currentLocale);
    ...
    getNextPageParam: (lastPage) => {
      if (lastPage && lastPage.hasMore && lastPage.page < lastPage.totalPages) {
  ...
  // Automatically fetch next page to achieve "load all" behavior
  useEffect(() => {
  ...
  const totalPokemon = data?.pages[0]?.total || 0;
  const isFullyLoaded = !hasNextPage && status === 'success';
  ...
  }, [isLoading, status, data, allPokemon.length, isFetchingNextPage, isFetching, hasNextPage, currentLocale]);
    ...
    <Layout>
      <div className="min-h-screen bg-gray-100 dark:bg-dark-bg-primary py-8 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          {/* Loading State */}
          {isLoading || (pokemon.length === 0 && (isFetching || hasNextPage || status === 'pending')) ? (
